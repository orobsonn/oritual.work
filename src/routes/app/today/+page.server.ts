import { dailyEntries, tasks, habits, habitCompletions, users } from '$lib/server/db/schema';
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	// Buscar completude dos hábitos de hoje
	const habitIds = todayHabits.map((h) => h.id);
	const todayCompletions = habitIds.length > 0
		? await locals.db
				.select()
				.from(habitCompletions)
				.where(and(eq(habitCompletions.date, today)))
		: [];

	const habitsWithStatus = todayHabits.map((habit) => ({
		...habit,
		completedToday: todayCompletions.some((c) => c.habitId === habit.id && c.completed === 1)
	}));
